...
class User extends CI_Controller {
    ...
    function login() {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            if ($this->Mmembers->validate_login($username, $password) != FALSE) {
    ...
    public function cart($edit = false, $id = false) {
        if (isset($_SESSION['giohang'])) {
            if (isset($edit) || isset($id)) {
                if ($edit == "down") {
                    for ($i = 0; $i < count($_SESSION['giohang']); $i++) {
                        if ($_SESSION['giohang'][$i]['id'] == $id) {
                            if ($_SESSION['giohang'][$i]['number'] > 0) {
                                $_SESSION['giohang'][$i]['number'] --;
                            } else {
                                $_SESSION['giohang'][$i]['number'] = 0;
                            }
                        }
                    }
                }
...
//        redirect('user/cart', 'refresh');
    ...
    function create_account() {
        ...
        if ($this->form_validation->run() == TRUE) {
            $result = $this->Mmembers->validate_signup($_POST['username']);
            if ($result == TRUE) {
                if ($this->Mmembers->create_account($data)) {
